...
import com.armedia.acm.configserver.environment.mapper.CloudMapper.ValueMissingException;
...
public class CloudMapperTest
{
    ...
    @Test
    public void testFailIfMissing() throws Exception
    {
        CloudMapperProperties properties = new CloudMapperProperties();
        properties.setNamespace(CloudMapperTest.NAMESPACE);
        properties.setFailIfMissing(true);
        CloudMapper cloudMapper = new CloudMapper(CloudMapperTest.CLIENT, properties);
        cloudMapper.postConstruct();
        ...
        for (final V1Secret secret : CloudMapperTest.SECRETS.values())
        {
            ...
            for (final String key : data.keySet())
            {
                ...
                final String nonExistentSecret = String.format("non-existent-secret-%s", UUID.randomUUID());
                final String var2 = String.format("@{secret:%s:%s}", nonExistentSecret, key);
                Assertions.assertThrows(ValueMissingException.class,
                        () -> cloudMapper.map(String.format("%s-test-nonExistent-%s", secretName, key), var2),
                        nonExistentSecret);
            }

            {
                ...
                final String var = String.format("@{secret:%s:%s}", secretName, nonExistentKey);
                Assertions.assertThrows(ValueMissingException.class,
                        () -> cloudMapper.map(String.format("%s-test-nonExistent", secretName), var),
                        nonExistentKey);
            }
        }
        ...
        for (final V1ConfigMap configMap : CloudMapperTest.CONFIG_MAPS.values())
        {
...

            {
                ...
                final String var = String.format("@{config:%s:%s}", configMapName, nonExistentKey);
                Assertions.assertThrows(ValueMissingException.class,
                        () -> cloudMapper.map(String.format("%s-test-nonExistent", configMapName), var),
                        nonExistentKey);
            }
        }
    }
